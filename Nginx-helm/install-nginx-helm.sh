...
az acr login -n anuchunduridevopsacr -u anuchunduridevopsacr -p ZawHNJfaNZjnpNEgX2wy3JvAJ533SOuieC8FdwEAZ5+ACRAoM1KD
...
#todo (pending): terraform code to assign network contributor and reader role to vnet for AKS and its managed identity.
#todo (pending): terraform code to deploy frontdoor and test frontdoor -> nginx -> istio -> app pod
#todo (pending): try the same with istio add-on instead of installing istio with helm
